class EventsController < ApplicationController
	...
	def index
		@events = Event.all.where("date >= current_date").order(date: :asc)
		build_park_array
		@type = "index"
		@title = "All Events"
	end
  ...
  def my_joined_events
  	@events = Event.all.where("creator_id = #{current_user.id}").where("date >= current_date").order(date: :asc)
  	@title = "My Joined Events"
  	render "index"
  end

  def my_created_events
  	@events = current_user.events.where("date >= current_date").order(date: :asc)
  ...
  def this_park_events
  	...
  	@events = Event.all.where("park_id = #{this_park_id}").where("date >= current_date")
